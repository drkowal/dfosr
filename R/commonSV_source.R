...
sampleCommonLogVols = function(h_y, h_prev, h_mu, h_phi, h_sigma_eta){
  ...
  #z = draw.indicators(res = ystar - matrix(rep(h_prev, m), nrow = n), nmix = list(m = m_st, v = v_st2, p = q))
  z = sapply(ystar-matrix(rep(h_prev, m), nrow = n), ncind, m_st, sqrt(v_st2), q)
  ...
  QHt.Matrix = bandSparse(n, k = c(0,1),
                          diagonals = list(rowSums(1/v_st2_all) + 1/sigmat2 + c(h_phi^2/sigmat2[-1], 0),
                                      -h_phi/sigmat2[-1]), symmetric = TRUE)
  ...
  # Sample the log-vols:
  hsamp = h_mu + matrix(Matrix::solve(chQht_Matrix,Matrix::solve(Matrix::t(chQht_Matrix), linht) + rnorm(n)), nrow = n)
...
initCommonSV = function(omega){
  ...
  ht = rowMeans(log(omega^2 + 0.0001), na.rm=TRUE)
